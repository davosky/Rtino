...
class StructuresControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create structure" do
    assert_difference('Structure.count') do
      post structures_url, params: { structure: { name: @structure.name, position: @structure.position } }
    end
  ...
  test "should update structure" do
    patch structure_url(@structure), params: { structure: { name: @structure.name, position: @structure.position } }
