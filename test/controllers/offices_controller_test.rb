...
class OfficesControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create office" do
    assert_difference('Office.count') do
      post offices_url, params: { office: { name: @office.name, position: @office.position } }
    end
  ...
  test "should update office" do
    patch office_url(@office), params: { office: { name: @office.name, position: @office.position } }
